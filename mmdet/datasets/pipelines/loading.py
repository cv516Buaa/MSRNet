...
class LoadAnnotations:
    ...
    def _load_density_map(self, results):
        """Private function to load semantic segmentation annotations.
        ...
        filename = osp.join(results['density_prefix'],
                            results['ann_info']['seg_map'])
        img_bytes = self.file_client.get(filename)
        results['gt_density_seg'] = mmcv.imfrombytes(
            img_bytes, flag='unchanged').squeeze()
        #print(results['gt_density_seg'].shape)
        results['seg_fields'].append('gt_density_seg')
        return results

    def _load_center_map(self, results):
        """Private function to load semantic segmentation annotations.
        ...
        filename = osp.join(results['center_prefix'],
                            results['ann_info']['seg_map'])
        img_bytes = self.file_client.get(filename)
        results['gt_ct_seg'] = mmcv.imfrombytes(
            img_bytes, flag='unchanged').squeeze()
        #print(results['gt_density_seg'].shape)
        results['seg_fields'].append('gt_ct_seg')
        return results
    ...
    def __call__(self, results):
        ...
        if self.with_density:
            results = self._load_density_map(results)
        if self.with_center:
            results = self._load_center_map(results)
        return results
    ...
    def __repr__(self):
        ...
        repr_str += f'with_density={self.with_density}, '
