...
from ..builder import HEADS, build_head, build_roi_extractor, DETECTORS, build_backbone, build_neck
...
class SCNetRoIHead(CascadeRoIHead):
    ...
    def __init__(self,
        ...
        if density_head is not None:
            self.density_roi_extractor = build_roi_extractor(
                density_roi_extractor)
            self.density_head = build_head(density_head)
    ...
    def with_density(self):
        """bool: whether the head has density head"""
        #print(self.density_head)
        return hasattr(self,
                       'density_head') and self.density_head is not None
    ...
    def _bbox_forward(self,
        ...
        if self.with_semantic and semantic_feat is not None:
            ...
            #draw_feature_map(semantic_feat)
    ...
    def density_train(self,
                      ...
                      gt_density_seg=None,
                      gt_ct_seg=None):
                      #gt_density_seg_2=None
        from .feature_visualization import draw_feature_map
        losses = dict()
        density_pred, fusion_pred = self.density_head(x)
        ct_pred, hw_pred = self.center_head(x)
        loss_seg_d = self.density_head.loss(density_pred, gt_density_seg)
        loss_seg_b = self.center_head.loss_center(ct_pred, gt_ct_seg[:,:,:,0])
        loss_hw = self.center_head.loss_hw(hw_pred, gt_ct_seg[:,:,:,1:3])
        losses['loss_density_seg'] = loss_seg_d
        losses['loss_density_seg_b'] = loss_seg_b
        losses['loss_hw'] = loss_hw

        return losses, density_pred, ct_pred
    #    return losses, density_pred, None
    ...
    def density_test(self, x, img_metas, rescale=False):
        #_, x, _ = self.fpn_2_head(x)
        density_pred, fusion_pred = self.density_head(x)
        ct_pred, hw_pred = self.center_head(x)
        from .feature_visualization import draw_feature_map
        #draw_feature_map(ct_pred)
        return density_pred, ct_pred
...
#//////////////////////////////////
        ...
        # return results
#        for i in range(len(det_bbox_results)):
#            bbox = det_bbox_results[i]
#            del_a = []
#            for j in range(bbox[0].shape[0]):
#                x_center = (bbox[0][j][0] + bbox[0][j][2])/2
#                y_center = (bbox[0][j][1] + bbox[0][j][3])/2
#                from .feature_visualization import draw_feature_map
#                x_reshape = (img_shapes[0][0] / scale_factors[0][0])/density_pred.shape[2]
#                y_reshape = (img_shapes[0][1] / scale_factors[0][0])/density_pred.shape[3]
#                if int(y_center/y_reshape)+1 < density_pred.shape[2] and  int(x_center/x_reshape)+1 < density_pred.shape[3]:
#                    if density_pred[0,0,int(y_center/y_reshape), int(x_center/x_reshape)] < 5:
#                        del_a.append(j)
#            det_bbox_results[i][0] = np.delete(det_bbox_results[i][0], del_a,axis=0)
        #det_bbox_results = torch.from_numpy(np.array(det_bbox_results)[:,0,:,:])
        #density_pred = F.interpolate(density_pred ,scale_factor=(8,8), mode='bilinear')
        #self.rpn2_head.loss(proposal_list, density_pred)
        if self.with_mask:
